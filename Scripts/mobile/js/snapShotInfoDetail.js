...
function ConfirmedFun(arry){
	...
	var result = alertMsg[snapShot_index].obj;
	var html =	`
		...
		<div class="title_2 row"><label class="col-100">事件</label><div class="col-100 snapshotEvent"><div>${alertMsg[snapShot_index].msg}</div></div></div> 
		...
		<a href="#" class="button button-big button-fill color-blue" onclick="OnSureMessage(${arry[2]},'${result.Time}',this)" values="${result}" title="${result.User_Confirmed}  ${formatDate(result.Dt_Confirmed)}">确定</a>
		</div>`; 
...
function NotConfirmedFun(arry){
	...
	var html =	`
				...
				<div class="title_2 row"><label class="col-40">时间</label><label class="col-60">${formatDate(result.Time)}</label></div> 
				<div class="title_2 row"><label class="col-100">事件</label><div class="col-100 snapshotEvent"><div>${alertMsg[snapShot_index].msg}</div></div></div> 
				<div class="title_2 row"><label class="col-100">处理意见</label><div class="snapshotEvent"><div>${alertMsg[snapShot_index].adMsg}</div></div></div> 
