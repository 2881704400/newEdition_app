...
//添加节点到html
function treeHTML_en(len, name, equip_no, thisDom) {
    var newRow = "";
    if (len > 0) {
        var alarm = selectAlarm_en(name);
        var alarmClass = '';
        if (alarm > 0) {
            alarmClass = 'alarm';
        } else {
        ...
        newRow = `<li class="accordion-item" onclick="onTreePar_en(this,'${name}',event)">
        ...
        if (Browse_Equip_List(equip_no) || Browse_SpecialEquip_List(equip_no, false)) {
            for (var i = 0; i < AllEquipStat.length; i++) {
                ...
                var iconColorClass = getIconColor_en(allEquipStat[2]);
...
function onTreePar_en(dt, name, e) {

    var doms = selectDom_en(name); 
    $(dt).find("ul").html("");
    doms.each(function() {
        ...
        treeHTML_en(len, name, equip_no, $(dt).find("ul"));
    });
