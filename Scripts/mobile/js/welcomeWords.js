...
function welcomeWords() {
    var url1 = WORDcommand.backgroundImage.url,
        ...
        setHtml = "",groupPhotoArray = new Array(),
        typeEnd = new Array();
        //获取文件夹图片
        $.when(AlarmCenterContext.GetFileStructure(url1,fileName1)).done(function(n){
            var dt = n.HttpData,
            ...
            if (result != "" && result != undefined && result != null) {
                for (var i = 0; i < result.length; i++) {
                    ...
                    if (i == 0) {
                        setHtml += '<div class="col-33"><img src="/Image/bg/' + groupPhotoArray[i] + "." + typeEnd[i] + '" data-bg="'+i+'"><i class="icon iconfont icongouxuan welcome-Word-control-icon-check"></i></div>';
                        $(".welcomeInput").css({
                          background: "url(/Image/bg/" + groupPhotoArray[i] + "." + typeEnd[i] + ") no-repeat center center/100% 100%"
                        });

                    } else {
                        setHtml += '<div class="col-33"><img data-bg="'+i+'" src="/Image/bg/' + groupPhotoArray[i] + "." + typeEnd[i] + '"><i></i></div>';
                    }
                }
                $(".welBgImage").html(setHtml+'<div class="col-33"></div><div class="col-33"></div>');
                $(".welBgImage>div").unbind().bind('click', function() {
                    $(this).find("i").addClass("icon iconfont icongouxuan welcome-Word-control-icon-check").parent().siblings().find("i").removeClass();
    ...
    $("#tab44 .welcome-Word-control-icon-mutual").unbind('click').bind('click', function() {
        var index = $(this).index(),equipNo = $(this).attr("set_equip"),setNo = $(this).attr("set_no");
        if (index == 0) {
            welcome_activeSave(this, 1);
            onSetCommand_return(this, equipNo, setNo, "");
        } else {
            onSetCommand_return(this, equipNo, setNo, "");
        }
    });
...
function welComeInit(){
        $.when(AlarmCenterContext.post("/api/GWServiceWebAPI/getWelcomeSpeech", {})).done(function(n) {
            if (n.HttpData.data.length > 0 && n.HttpData.code == 200) {
                ...
                if (result != "" && result != null && result != undefined) {
                    ...
                    //背景设置
                    $(".welBgImage>div").find("img[data-bg='"+result.databg+"']"). parent().find("i").addClass("icon iconfont icongouxuan welcome-Word-control-icon-check").parent().siblings().find("i").removeClass();
                    $(".welcomeInput").css({
...
function welcome_activeSave(that, number) {
    ...
    if (fileNameURL == undefined || fileNameURL == "none") {
        fileNameURL = $(".welBgImage>div:eq(0)").find("img").attr("src");
    }
    var fileName = fileNameURL.split("/")[fileNameURL.split("/").length - 1]; //name
    welcome_objoriginal.BackgroundImg = fileNameURL;
    welcome_objoriginal.databg = $(".welcome-Word-control-icon-check").parent().find("img").attr("data-bg");
    //是否居中
    welcome_objoriginal.center = "positionClass";
    // var allHTML = `<html><head><meta charset="utf-8"><meta http-equiv="Expires" content="0"><meta http-equiv="Cache" content="no-cache"><style type="text/css">*{margin: 0;padding: 0;}html,body{width: 100%;height: 100%;position: relative;overflow: hidden;}.a123 span{font-family:${welcome_objoriginal.FontFamily};position: absolute;white-space: pre;width: 100%;height: 100%;display: flex;} .positionClass{width: 100% !important;left: 0% !important;display: inline-block;text-align: center;padding: 11px;transform: translateX(-50%);transform: translateY(-50%);}</style></head><body><div style="width: 100%;height: 100%;text-align: center;background: url(${fileNameURL.replace("url(", "").replace(")", "")}) no-repeat center center/100%;" class="a123">"<span style="font-size: ${welcome_objoriginal.FontSize}px;color:${welcome_objoriginal.FontColor}; justify-content:${welcome_objoriginal.CanvasLeft}; align-items:${welcome_objoriginal.CanvasTop}; font-weight:${welcome_objoriginal.FontWeight}; font-style:${welcome_objoriginal.FontStyle};">${welcomeVal}</span></div></body></html>`;
 var allHTML = "<html>" + "<head>" + "<meta charset=\"utf-8\">" +"<meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\">"+ "<meta http-equiv=\"Expires\" content=\"0\">" + "<meta http-equiv=\"Cache\" content=\"no-cache\">" + "<title>欢迎词</title>" + "<style type=\"text/css\">" + "*{margin: 0;padding: 0;}" + "html,body{width: 100%;height: 100%;position: relative;overflow: hidden;}" + ".a123 span{  font-family:" + welcome_objoriginal.FontFamily + ";position: absolute;white-space: pre;width: 100%;height: 100%;display: flex;}"  + "</style>" + "</head>" + "<body>" + "<div style=\"width: 100%;height: 100%;text-align: center;background: url(" + fileNameURL.replace("url(", "").replace(")", "") + ") no-repeat center center/100%;\" class=\"a123\">" + "<span style=\"font-size: " + welcome_objoriginal.FontSize + "px;color: " + welcome_objoriginal.FontColor + "; justify-content: " + welcome_objoriginal.CanvasLeft + "; align-items: " + welcome_objoriginal.CanvasTop + "; font-weight: " + welcome_objoriginal.FontWeight + "; font-style: " + welcome_objoriginal.FontStyle + "; \">" + welcomeVal + "</span>" + "</div>" + "</body>" + "</html>";
 // + ".positionClass{width: 100% !important;left: 0% !important;display: inline-block;text-align: center;padding: 11px;transform: translateX(-50%);transform: translateY(-50%);}"
